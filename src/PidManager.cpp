...
PidManager::PidManager() { AUV_INFO("Constructor"); }

PidManager::PidManager(std::shared_ptr<ros::NodeHandle> nh) : nh_(nh) {

  // setZero all sensors

  //   ros::Subscriber vision_plant_sub =
  //   nh_->subscribe("/pid_plantState_vision", 100, visionPlant_callback);
  // //  ros::Subscriber imu_plant_sub = nh_->subscribe("pid_plantState_IMU",
  // 100, IMU_callback);

  //
  //
  // ros::Subscriber depth_plant_sub = nh_->subscribe("/pid_plantState_depth",
  // 100, depth_plant_callBack);
...
bool PidManager::getTimeout() { return bTimoutSwitchState_; }
...
void PidManager::taskDelay(int seconds) { ros::Duration(seconds).sleep(); }
