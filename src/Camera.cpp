...
void Camera::startRecording() {
  ...
  AUV_INFO("Writing Video to %s and %s", filenameVideoFront,
           filenameVideoBottom);

  if (isFrontOpened) {
    writerFront.open(filenameVideoFront, codeFourcc, camFPS,
                     cv::Size(imgWidth, imgHeight));
    isFrontRecording = true;
  }

  if (isBottomOpened) {
    writerBottom.open(filenameVideoBottom, codeFourcc, camFPS,
                      cv::Size(imgWidth, imgHeight));
...
cv::Mat Camera::getFront() { return lastFrontImage; }
cv::Mat Camera::getBottom() { return lastBottomImage; }
